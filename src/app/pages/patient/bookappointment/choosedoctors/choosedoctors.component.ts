...
export class ChoosedoctorsComponent implements OnInit {
  ...
  ngOnInit() {
    this.route.paramMap.subscribe((params) => {
      ...
      this.docService
        .getAllDoctorsByMedicareService(+params.get("id"))
        .subscribe((docs: Doctor[]) => (this.doctors = docs));
    });
    // });
  }
  ...
  bookNow(doc, medicare) {
    this.router.navigateByUrl(
      "/patient/bookappointment/bookappointmentform/" +
        medicare.medicareServiceId +
        "/" +
        doc.doctorId
    );
  }
